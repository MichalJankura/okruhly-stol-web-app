    ...
    const fetchFilterOptions = async () => {
      try {
        const [yearsRes, monthsRes, categoriesRes, authorsRes] = await Promise.all([
          ...
          fetch('http://localhost:5000/api/months').then(res => res.json()),
  ...
  useEffect(() => {
    const fetchArticles = async () => {
      ...
      try {
        ...
        console.log('Fetching from URL:', `http://localhost:5000/api/blog-posts?${queryParams}`);
        const response = await fetch(`http://localhost:5000/api/blog-posts?${queryParams}`);
